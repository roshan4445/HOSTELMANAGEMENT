...
const Payments = () => {
          ...
          <button 
            ...
            onClick={async () => {
              ...
              try {
                const token = JSON.parse(localStorage.getItem('userInfo')).token;
                const config = { headers: { Authorization: `Bearer ${token}` } };
                const { data } = await axios.post('http://localhost:5000/api/payments/generate', {}, config);
  ...
  const handleMarkPaid = async (e) => {
    ...
    try {
      const token = JSON.parse(localStorage.getItem('userInfo')).token;
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.put(`http://localhost:5000/api/payments/${selectedPayment._id}/pay`, {
        ...
        <div className="flex flex-wrap gap-3 w-full sm:w-auto">
          ...
          <motion.button 
            ...
            onClick={async () => {
              try {
                const token = JSON.parse(localStorage.getItem('userInfo')).token;
                await axios.post('http://localhost:5000/api/payments/test-overdue', {}, { headers: { Authorization: `Bearer ${token}` } });
