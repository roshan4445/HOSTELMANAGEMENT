...
const Complaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/complaints`, config);
      setComplaints(data);
      setLoading(false);
    } catch (error) {
      console.error(error);
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const token = JSON.parse(localStorage.getItem('userInfo')).token;
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.put(`${import.meta.env.VITE_SERVER_URL}/api/complaints/${id}`, { status: newStatus }, config);
