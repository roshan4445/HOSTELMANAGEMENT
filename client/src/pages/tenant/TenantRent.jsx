...
const TenantRent = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Rent History</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">View and manage your rent payments</p>
      </div>
      ...
      <div className="space-y-4">
        {payments.map((payment) => (
          <motion.div
            key={payment._id}
            layout
            className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700/50 flex flex-col gap-4 relative overflow-hidden"
          >
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="font-bold text-gray-800 dark:text-gray-100 text-lg">{format(new Date(payment.year, payment.month - 1), 'MMMM yyyy')}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5">Due: {format(new Date(payment.dueDate), 'dd MMM yyyy')}</p>
              </div>
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-slate-700/50 grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Amount</p>
                <p className="text-lg font-black text-gray-800 dark:text-gray-100">₹{payment.total}</p>
              ...
              {payment.status === 'Paid' ? (
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{payment.paymentDate ? format(new Date(payment.paymentDate), 'dd MMM yyyy') : '-'}</p>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mt-0.5">{payment.paymentMode}</p>
                </div>
              ) : (
        ...
        {payments.length === 0 && (
          <div className="text-center py-10 bg-white dark:bg-slate-800 rounded-3xl border border-gray-100 dark:border-slate-700/50">
            <p className="text-gray-500 dark:text-gray-400 font-medium">No rent history found.</p>
          </div>
        )}
      </div>
        ...
        {showConfirm && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <motion.div
              ...
              className="bg-white dark:bg-slate-800 p-6 rounded-2xl w-full max-w-[380px] shadow-xl"
            >
              <div className="text-center mb-5">
                ...
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Confirm Payment</h2>
              ...
              <div className="mb-5">
                ...
                <select
                  className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-xl px-4 py-3 outline-none focus:bg-white dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  value={selectedMode}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-slate-800/50 text-gray-700 dark:text-gray-200 font-semibold rounded-xl hover:bg-gray-200 transition-colors"
                >
                  Cancel
                </button>
                <motion.button
